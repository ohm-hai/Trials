"""Phase 2d: FP8-weight N-split decomposition for GLM-5.2 MoE GEMM.
FP8 (e4m3) weights halve per-expert per-XCD L2 footprint:
  bf16 3.14MB -> FP8 1.57MB (matches proven 1.94x regime)."""
...
@triton.jit
def _moe_fp8_kernel(
    a_ptr, b_ptr, b_scale_ptr, c_ptr, sti_ptr, eid_ptr, rowstart_ptr, npp_ptr, nvt,
    N, K, EM,
    ...
    for kk in range(0, tl.cdiv(K, BK)):
        ...
        b_fp8 = tl.load(bp)
        b = (b_fp8.to(tl.float32) * b_scale).to(tl.bfloat16)
        acc = tl.dot(a, b, acc)
        ap += BK * sak
        bp += BK * sbk
...
def bench(M_tokens, mode, warmup=15, iters=40):
    ...
    def run():
        ...
        args = (a, b_fp8, b_scale, c, sti, eid, pid_remap, npp_t, nvt,
                N, K, E, a.stride(0), a.stride(1), b_fp8.stride(0), b_fp8.stride(1), b_fp8.stride(2),
                c.stride(0), c.stride(1), BLOCK_M, BLOCK_N, BLOCK_K, GROUP_M, mode, NUM_XCD, TOPK,
                NUM_NB_PER_SLICE)
        for _ in range(warmup):
            _moe_fp8_kernel[grid](*args)
...
def main():
    print(f"=== Phase 2d: FP8 N-split decomposition (GLM-5.2, fp8 e4m3 weights) ===")
    print(f"K={K} N={N} E={E} topk={TOPK} NUM_XCD={NUM_XCD}")
    print(f"per-expert per-XCD fp8 weight = {K*N_SLICE*1/1e6:.3f} MB (fits 4MB L2: {K*N_SLICE < 4e6})\n")
